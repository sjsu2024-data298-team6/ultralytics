...
from ultralytics.nn.modules.ijepa.src.utils.tensors import trunc_normal_, repeat_interleave_batch
...
class ConvEmbed(nn.Module):
    ...
    def __init__(self, channels, strides, img_size=224, in_chans=3, batch_norm=True):
        ...
        for i in range(len(channels) - 2):
            stem += [
                nn.Conv2d(
                    channels[i], channels[i + 1], kernel_size=3, stride=strides[i], padding=1, bias=(not batch_norm)
                )
            ]
            if batch_norm:
